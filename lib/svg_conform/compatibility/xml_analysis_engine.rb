  ...
  module Compatibility
    ...
    class XmlAnalysisEngine
      ...
      def display_analysis_header(filename, element_count)
        ...
        puts Paint["#{mode_icon} XML File #{mode_text}: #{filename}", :cyan, :bold]
      ...
      def process_svg_elements(svg_elements, filename, svgcheck_report)
        svg_elements.each_with_index do |svg_element, index|
          ...
          process_single_svg_element(svg_element, filename, index, svgcheck_report)
        end
      end

      def process_single_svg_element(svg_element, filename, index, svgcheck_report)
        ...
        begin
          ...
          if @context.repair_mode?
            process_embedded_svg_repair(element_filename, svg_content, svgcheck_report)
          else
            process_embedded_svg_validation(element_filename, temp_file.path, svgcheck_report)
          end
        ensure
          temp_file.unlink
        end
      ...
      def process_embedded_svg_validation(element_filename, temp_file_path, svgcheck_report)
        svg_conform_result = run_validation(temp_file_path)
        svg_conform_report = create_conformance_report(element_filename, svg_conform_result)

        if @context.semantic_analysis?
          build_semantic_validation_result(element_filename, svg_conform_report, svgcheck_report)
        else
          build_basic_validation_result(element_filename, svg_conform_report, svgcheck_report)
        end
      end

      def process_embedded_svg_repair(element_filename, svg_content, svgcheck_report)
        remediation_runner = SvgConform::RemediationRunner.new(profile: @context.profile)
        svg_conform_result = remediation_runner.run_remediation(svg_content, filename: element_filename)

        if @context.semantic_analysis?
          build_semantic_repair_result(element_filename, svg_conform_result, svgcheck_report)
        else
          build_basic_repair_result(element_filename, svg_conform_result, svgcheck_report)
        end
      end
