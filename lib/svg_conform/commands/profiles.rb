  ...
  module Commands
    ...
    class Profiles
      ...
      def load_available_profiles
        ...
        profile_dir = File.join(File.dirname(__FILE__), '../../../config/profiles')
        ...
        profile_files.each do |file|
          ...
          begin
            ...
            puts Paint["Warning: Could not load profile '#{name}': #{e.message}", :yellow]
          end
        end
      ...
      def display_profile_list(profiles)
        ...
        table = TableTennis.new(table_data, headers: { profile: 'Profile', description: 'Description' })
        puts table
        puts
        puts Paint['Use --verbose for detailed information about each profile', :black]
      end
