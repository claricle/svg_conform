...
module SvgConform
  ...
  class ReportComparator
    def initialize(profile: :svg_1_2_rfc, reports_dir: 'spec/fixtures/svgcheck_reports')
      @profile = profile
    ...
    def compare_reports(svg_conform_report, svgcheck_report, filename)
      ...
      result = generate_comparison_result(svg_conform_report, svgcheck_report, filename)
    ...
    def compare_single_file(filename)
      ...
      begin
        ...
        svgcheck_report_path = File.join(@reports_dir, "#{basename}.svgcheck.yaml")
        ...
        generate_comparison_result(svg_conform_report, svgcheck_report, basename)
    ...
    def compare_all_test_files
      puts "\n#{Paint['=' * 100, :cyan]}"
      puts Paint['🔍 COMPREHENSIVE SVGCHECK COMPATIBILITY ANALYSIS', :magenta, :bold]
      puts Paint['=' * 100, :cyan]

      test_files = Dir.glob(File.join(@reports_dir, '*.svgcheck.yaml')).map do |report_file|
      ...
      test_files.each do |filename|
        ...
        begin
          # Load svgcheck report
          svgcheck_report_path = File.join(@reports_dir, "#{filename}.svgcheck.yaml")
          # Use the new external checker parser
          parser = SvgConform::ExternalCheckers::Svgcheck::Parser.new
          error_content = File.read(svgcheck_report_path)
    ...
    def generate_comparison_result(svg_conform_report, svgcheck_report, filename)
    ...
    def group_errors_by_semantic_meaning(errors)
      errors.group_by { |error| extract_semantic_meaning(error) }.transform_values(&:length)
    end
    ...
    def determine_color_scale_options(headers, _rows)
      ...
      if headers.include?('Match?') || headers.include?('Status')
        ...
        if count_columns.any?
          ...
          column_name = headers[count_columns.first].downcase.gsub(/\s+/, '_').to_sym
          {
    ...
    def apply_default_color_scale(cell_str)
      ...
      if cell_str.include?('true') || cell_str.include?('false')
        cell_str.include?('true') ? Paint[cell_str, :green] : Paint[cell_str, :red]
    ...
    def display_requirement_mapping(result)
      ...
      begin
        ...
        unmapped_svgcheck_errors = identify_unmapped_svgcheck_errors(svgcheck_report, svg_conform_report)
          ...
          all_requirements.each do |requirement|
            ...
            if svgcheck_errors.any?
              svgcheck_errors.each do |error|
                puts "    #{Paint['↳ [svgcheck]', :magenta]} #{Paint[error.message, :magenta]}"
              end
            else
        ...
        if unmapped_svgcheck_errors.any?
          ...
          puts Paint['These svgcheck errors do not correspond to any SvgConform requirement:', :red]
          unmapped_svgcheck_errors.each do |error|
            puts "    #{Paint['⚠️ [svgcheck]', :red, :bold]} #{Paint[error.message, :red]}"
          end
        end
        ...
        puts "\n#{Paint['🎉 ✓ ERROR MESSAGES MATCH PERFECTLY! 🎉', :green, :bold]}"
      end
    end
    ...
    def determine_coverage_status(semantic_mapping)
      ...
      svg_conform_errors = semantic_mapping.values.map { |counts| counts[:svg_conform] }.sum
      svgcheck_errors = semantic_mapping.values.map { |counts| counts[:svgcheck] }.sum
    ...
    def display_overall_summary(results)
      ...
      rows << ['---', '---', '---', '---']
      rows << ['TOTAL', "#{total_files} files", "#{matching_files} matches", "#{match_percentage}%"]

      render_gh_style_table(headers, rows)

      puts "\n#{Paint['📊 COMPATIBILITY METRICS:', :blue, :bold]}"
      puts "  🗂️  Total files processed: #{Paint[total_files.to_s, :cyan, :bold]}"
      puts "  ✅ Files with matching error counts: #{Paint[matching_files.to_s, :green, :bold]}"
      ...
      puts "  📈 Compatibility percentage: #{Paint["#{match_percentage}%", percentage_color, :bold]}"
    end
  end
end
