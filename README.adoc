...
=== `svg_conform validate`

The `validate` command allows you to validate SVG files against a specified profile.
...
The `fix` command allows you to automatically fix SVG files to conform to a
specified profile, with options for in-place modification or custom output.

Whether a file can be fixed depends on the specific issues present and the
remediations available in the selected profile.
...
. svgcheck outputs are generated for test files using the `generate-svgcheck` command.

. For each test file, svgcheck outputs are generated in both check-only and
repair modes through the `generate-svgcheck` command, and are then parsed by the
...
The `compatibility` command performs a comprehensive compatibility analysis
...
The `generate-svgcheck` command generates svgcheck outputs for test files to
